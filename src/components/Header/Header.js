        ...
        <header className="header">
            <div className="header__wrapper container">
                    ...
                    {menu.map(item => {
                        return (
                            <li className='header__menu-item' key={item.id}>
                                <Link to={item.to} className={'header__menu-link' + (checkUrl(item.to) ? " active" : "")} onClick={mobileScreen ? () => setMenuVisible(!menuVisible) : () => false}>{item.title}</Link>
                            </li>
                        );
                    })}
                </ul>
                <div className="header__wrapper-inner">
                    ...
                    {accounts[0] ?
                        <button className='button button--header button--white'>
