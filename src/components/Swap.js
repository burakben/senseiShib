...
export default function Swap({ walletType, userAddress, setPopupShow }) {
    ...
    const getUserBalance = async () => {
        if (userAddress) {
            ...
            setInBalance(
                Moralis.Units.FromWei(inBalance, trade.tokenIn.decimals).toFixed(4)
            );
            setOutBalance(
                Moralis.Units.FromWei(outBalance, trade.tokenOut.decimals).toFixed(4)
            );
        }
    };
    ...
    const getExchangeRate = async () => {
        ...
        if (result) {
            ...
            const rate =
                Moralis.Units.Token(tokenAreserve, trade.tokenOut.decimals) /
                Moralis.Units.Token(tokenBreserve, trade.tokenIn.decimals);

            let amountIn = Moralis.Units.Token(
                trade.amountIn,
            ...
            const exchangeRate = `${rate.toFixed(10)} ${trade.tokenIn.symbol} per ${trade.tokenOut.symbol
                }`;
            setExchangeText(exchangeRate);
            setImpact(impact.toFixed(2));
        } else {
            const value = Moralis.Units.Token("1", trade.tokenOut.decimals);
            let rate = await quote(value, "to");
            const exchangeRate = `${Number(rate).toFixed(10)} ${trade.tokenIn.symbol
